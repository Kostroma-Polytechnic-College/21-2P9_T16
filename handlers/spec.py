...
async def speciality_kb(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == 'Ar':
        ...
        await call.message.answer("Область профессиональной деятельности выпускников:"
...
,reply_markup= SpecConfKeyBoard())
        ...
        await state.update_data(spec=call.data)
        await call.message.answer("область профессиональной деятельности выпускников – выполнение гидрогеологических и инженерно-геологических работ;"
...
"выполнение работ по профессии «Рабочий на геологических работах»",reply_markup= SpecConfKeyBoard())
        ...
        await state.update_data(spec=call.data)
        await call.message.edit_reply_markup(reply_markup=None)
        await call.message.answer("Студенты данной специальности учатся следующему:"
...
"Разрабатывают тестовые наборы и тестовые сценарии.",reply_markup= SpecConfKeyBoard())
        ...
        await state.update_data(spec=call.data)
        await call.message.answer("Организация и проведение работ по монтажу,"
"сборке и демонтажу электронных приборов и устройств - выполнение сборки,"
"монтажа и демонтажа электронных приборов и устройств," 
"разработка технологических процессов по монтажу," 
"и сборке электронных приборов и устройств," 
"подбор технологического оборудования для проведения сборочно – монтажных работ."
"Организация работ по выполнению настройки,"
"регулировки и проведению испытаний электронных приборов и устройств -"
"осуществление выбора измерительных приборов и оборудования для проведения испытаний электронных приборов и устройств,"
"разработка технологических процессов по настройке и регулировке электронных приборов и устройств."
"Организация работ по проведению технического обслуживания и ремонта электронных приборов и устройств -"
"осуществление анализа электрических схем электронных приборов и устройств,"
"составление алгоритмов диагностирования электронных приборов и устройств," 
"проведение технического обслуживания и ремонта электронных приборов и устройств.",reply_markup= SpecConfKeyBoard())
        await call.message.edit_reply_markup(reply_markup=None)
        await state.set_state(RegisterState.cnfKbSpecState)
                        
    elif call.data == 'Ra':
        await state.update_data(spec=call.data)
        await call.message.answer("Область профессиональной деятельности выпускников: организация и проведение работ по монтажу, ремонту, эксплуатации и техническому обслуживанию различных видов радиоэлектронной техники."
"Объекты профессиональной деятельности выпускников:"
...
"Квалификация выпускника углубленной подготовки — старший техник.",reply_markup= SpecConfKeyBoard())
        ...
        await state.update_data(spec=call.data)
        await call.message.answer("Специальность «Разработка электронных устройств и систем» подходит для тех, кто хочет изобретать новые устройства."
...
"работать со схемами и специальными инструментами.",reply_markup= SpecConfKeyBoard())
        ...
        await state.update_data(spec=call.data)
        await call.message.answer("Цифровой куратор — специалист в области компьютерной грамотности, помогающий гражданам получить навыки работы с компьютером, оргтехникой и электронными ресурсами."
...
"подготовку презентационных материалов и компьютерного оборудованя;"
